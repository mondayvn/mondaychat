class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "mondaychat-bot-#{conversation.inbox.id}",
      channel_id: "mondaychat-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "mondaychat-csml-bot-#{agent_bot.id}",
      name: "mondaychat-csml-bot-#{agent_bot.id}",
      default_flow: 'mondaychat_bot_flow',
      flows: [
        {
          id: "mondaychat-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'mondaychat_bot_flow',
