...
RSpec.describe 'Confirmation Instructions' do
  describe :notify do
    ...
    it 'has the correct header data' do
      expect(mail.reply_to).to contain_exactly('accounts@monday.com.vn')
    ...
    it 'does not refer to the inviter and their account' do
      expect(mail.body).to_not match('has invited you to try out Mondaychat!')
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail.body).to match(
          "#{CGI.escapeHTML(inviter_val.name)}, with #{CGI.escapeHTML(account.name)}, has invited you to try out Mondaychat."
        )
